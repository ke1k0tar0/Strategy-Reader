...
// Micro-component for clipboard interaction and visual feedback
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-all opacity-0 group-hover:opacity-100 focus:opacity-100"
      title="Copy value"
    >
      {copied ? (
        <svg
  ...
  const renderParameterBlock = (data: any) => {
    return Object.entries(data).map(([key, value]) => {
      ...
      return (
        <div
          key={key}
          className="bg-slate-50/70 border border-slate-100 rounded-xl p-4 transition-colors hover:bg-slate-100 relative group"
        >
          <div
            className="text-xs font-medium text-slate-500 mb-1 truncate pr-6"
            title={key}
          >
            {key}
          </div>
          <div className="font-mono text-base font-semibold text-slate-800 break-all pr-6">
            {displayValue}
          </div>
          {/* Inject the copy button into the primitive card */}
